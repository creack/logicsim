...
const API_URL = "http://leto:4000";

const SignedEigthSegmentsDisplay: React.FC<{
  entity: EntityInstance;
  inputs?: IO[];
  x?: number;
  y?: number;
}> = ({ inputs, ...props }) => {
  const instance = props.entity.entities.find(
    (elem) => elem.root.title === "signed8segmentsdriver0"
  );
  if (!instance) {
    throw new Error(`entity 'signed8segmentsdriver0' not found`);
  }

  const signed = !!instance.root.inputs?.find(
    (elem) => elem.title === "rippleNegative"
  )?.value;

  return (
    <RawEightSegmentsDisplay
      {...props}
      inputs={instance.root.outputs}
      signed={signed}
    />
  );
};

const ThreeDigitDecimalDisplay: React.FC<{
  entity: EntityInstance;
  inputs?: IO[];
  x?: number;
  y?: number;
}> = ({ inputs, entity, x = 0, ...props }) => {
  const h = entity.entities.find(
    (elem) => elem.root.title === "signed8segments0"
  );
  if (!h?.root.inputs) {
    throw new Error(
      `entity 'signed8segments0' not found in 3digitDecimalDisplay`
    );
  }

  const t = entity.entities.find(
    (elem) => elem.root.title === "signed8segments1"
  );
  if (!t?.root.inputs) {
    throw new Error(
      `entity 'signed8segments1' not found in 3digitDecimalDisplay`
    );
  }

  const o = entity.entities.find(
    (elem) => elem.root.title === "signed8segments2"
  );
  if (!o?.root.inputs) {
    throw new Error(
      `entity 'signed8segments2' not found in 3digitDecimalDisplay`
    );
  }
...
const RawEightSegmentsDisplay: React.FC<{
  inputs?: IO[];
  x?: number;
  y?: number;
  signed?: boolean;
  ...
  return (
    <Group key={0} x={x + 15} y={y + 30}>
      <Rect
        ...
        height={10}
        fill={
          inputs.find((elem) => elem.title === "0")?.value
            ? enabledColor
            : disabledColor
        }
      />
      <Rect
        ...
        height={50}
        fill={
          inputs.find((elem) => elem.title === "1")?.value
            ? enabledColor
            : disabledColor
        }
      />
      <Rect
        ...
        height={50}
        fill={
          inputs.find((elem) => elem.title === "2")?.value
            ? enabledColor
            : disabledColor
        }
      />
      <Rect
        ...
        height={10}
        fill={
          inputs.find((elem) => elem.title === "3")?.value
            ? enabledColor
            : disabledColor
        }
      />
      <Rect
        ...
        height={50}
        fill={
          inputs.find((elem) => elem.title === "4")?.value
            ? enabledColor
            : disabledColor
        }
      />
      <Rect
        ...
        height={50}
        fill={
          inputs.find((elem) => elem.title === "5")?.value
            ? enabledColor
            : disabledColor
        }
      />
      <Rect
        ...
        height={10}
        fill={
          inputs.find((elem) => elem.title === "6")?.value
            ? enabledColor
            : disabledColor
        }
      />
      {signed ? (
        <Rect
          ...
          height={8}
          fill={
            inputs.find((elem) => elem.title === "7")?.value
              ? enabledColor
              : disabledColor
          }
        />
...
export const App1 = () => {
  const enableAxios = false;
  React.useEffect(() => {
    if (!enableAxios) return;
    axios.get(API_URL + "/").then((resp) => {
      console.log("Get resp:", resp);
      // setEntities(resp.data?.entities ?? []);
    });
  }, [enableAxios]);
  ...
  const pp = React.useMemo(() => {
    const pp = new EntityInstance(src);
          ...
          onClick={() => {
            setInputs(
              inputs.map((subelem) => ({
                ...subelem,
                value:
                  subelem.title === elem.title ? !subelem.value : subelem.value,
              }))
            );
          }}
        >
...
const library: () => Entity[] = () => [
  ...
  },
  {
    ...
    inputs: [{ title: "0" }],
    outputs: [{ title: "0" }],
    entities: [{ Type: "nand", title: "nand1", x: 40, y: 40 }],
    connections: [
      {
        From: {
          Type: "root",
...
class EntityInstance {
  ...
  constructor(root: Entity, indent: number = 0) {
    this.root = root;
    this.entities = (this.root.entities ?? []).map((elem) => {
      ...
      return new EntityInstance({ ...ret, ...elem }, indent + 1);
    });
...
const Foo: React.FC<{ pp: EntityInstance }> = ({ pp }) => {
...
const MyRect: React.FC<{
  ...
  const ioRadius = heightOffset / 4;

  const maxLen = Math.max(inputs.length, outputs.length);
  const height = propHeight ?? (maxLen + 1) * heightOffset;

  const handleOnDragStartRect = (_: KonvaEventObject<DragEvent>) => {
    setRect({ ...rect, isDragging: true });
  };

  const handleOnDragEndRect = (e: KonvaEventObject<DragEvent>) => {
    setRect({
      ...rect,
      isDragging: false,
      x: e.target.x(),
